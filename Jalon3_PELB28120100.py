...
L = 0.25  # longeur Dimension = du mur (m)
l = 0.2  # Largeur 
h = 0.2 # Hauteur
S = h * l  # Surface
N = 10000  # Nombre de volumes finis
...
dt = 300  # Intervalle de temps en secondes (15 minutes)

T_old = np.ones(N) * T_init  # Température initiale
T_new = np.copy(T_old)
...
x = np.linspace(0, L, N)
temps = []
print("Bonjour bienvenu dans la simulation de Benjamin PELLIEUX Dirichlet-Dirichlet")
...
for i in range(0, t_total, dt):
    ...
    for j in range(N):
        try:
            ...
            print(f"[ERROR] IndexError in T vector")
            exit(-1)
